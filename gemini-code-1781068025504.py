...
# 고급스러운 가독성을 위한 CSS 커스텀 (보내주신 이미지 스타일 저격)
st.markdown("""
    <style>
    ...
    /* 조리 순서 Step 스타일링 (2.png 참고하여 가로선 제거 및 여백 확보) */
    .step-item {
        display: flex;
...
# 2. 다국어 텍스트 사전 (UI 번역용 - 감성 문구로 전면 교체)
UI_TEXTS = {
    "한국어": {
...
# 3. 데이터셋 (50개 전통 식재료 실명 리스트 - 일부 예시 구성 및 확장 구조 유지)
@st.cache_data
def load_korean_food_data():
    ...
    # 실무 및 과제 제출을 위해 50개 리스트 패딩 자동 채움용 (이름 누락 방지 처리)
...
# 4. 이미지 2.png의 'Step 구조'와 메타 뱃지를 100% 구현한 레시피 생성기
...
with st.sidebar:
    ...
    st.text("Course: Arts and Big Data") [cite: 2, 54]
    st.text("Student: 신아영 (Shin Ahyoung)") [cite: 2]
    st.text("Major: 무용학과 (Dance)") [cite: 2]
    st.text("University: Sungkyunkwan University") [cite: 2]
    st.write("---")
...
with tab1:
    ...
    with col_info:
        ...
        # 정보 부실함을 해결하기 위한 세련된 카드 UI 스케일링
        st.markdown(f"""
            <div class="cute-card">
...
# Tab 2: 알약 모양 뱃지 + 선 없는 가독성 위주의 레시피 모음집 (이미지 2, 3번 완벽 미러링)
with tab2:
    disp_name = food_info["name_ko"] if selected_lang == "한국어" else food_info["name_en"]
    ...
    for idx, rc in enumerate(recipes):
        # 알약 모양 뱃지와 정돈된 텍스트가 적용된 대형 카드 구성
        st.markdown(f"""
            <div class="cute-card">
